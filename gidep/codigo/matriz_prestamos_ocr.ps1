...
$records = @()
foreach ($item in Get-ChildItem -LiteralPath $resolvedDirectory -Filter '*.png' | Sort-Object Name) {
    $file = Await-WinRt (
    ...
    try {
        ...
        try {
            ...
            $records += [pscustomobject]@{ name = $item.BaseName; text = [string]$result.Text }
        }
        finally {

...
def reporte_xlsx(filas, ruta):
    ...
    tiene_nota = any(len(fila) > 6 and str(fila[6] or "").strip() for fila in filas)
    enc = ["Acreedor", "Concepto", "MEF (USD)", "BCE (USD)", "Diferencia", "Estado"]
    if tiene_nota:
        enc.append("Nota / Ajuste")
    ws.append(enc)
    ...
    for fila in filas:
        salida = list(fila[:6])
        if tiene_nota:
            salida.append(fila[6] if len(fila) > 6 else "")
        ws.append(salida)
        r = ws.max_row
        for cell in ws[r]:
            cell.border = borde
    ...
    anchos = [34, 22, 16, 16, 16, 14, 72] if tiene_nota else [34, 22, 16, 16, 16, 14]
